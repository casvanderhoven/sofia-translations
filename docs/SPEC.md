...
**Deployment (24 Sep 2026):** Cloudflare Pages project `sofia-translations` (direct upload; wrangler's new Workers delegation refuses monorepo roots, so the project was created with `--force`). Production URL https://sofia-translations.pages.dev, custom domain `sofia.casvanderhoven.com` attached. `pnpm deploy` builds and uploads with the local wrangler login. `.github/workflows/deploy.yml` deploys on push to main once `CLOUDFLARE_API_TOKEN` (Pages: Edit) is set as a repo secret; `CLOUDFLARE_ACCOUNT_ID` is already set.
